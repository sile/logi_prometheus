...
-type table_key() :: {registry(), logi:logger(), logi:severity(), atom(), module()}.
...
-spec increment(Key, InitialDelay) -> ok when
      Key :: table_key(),
      InitialDelay :: logi_prometheus_backend:non_neg_milliseconds().
increment(Key, InitialDelay) ->
    {Registry, Logger, Severity, App, Module} = Key,
    Labels = [Logger, Severity, App, Module],
...
-spec handle_delayed_increment(Arg, #?STATE{}) -> #?STATE{} when
      Arg :: {table_key(), logi_prometheus_backend:non_neg_milliseconds()}.
